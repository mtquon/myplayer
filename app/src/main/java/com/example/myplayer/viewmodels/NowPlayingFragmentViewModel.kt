...
class NowPlayingFragmentViewModel(
    ...
    /**
     * Utility class used to represent the metadata necessary to display the media item currently
     * being played
     */
    ...
    data class NowPlayingMetadata(
        ...
        companion object{
            /**
             * Utility method to convert miliseconds to a display of minutes and seconds
             */
            fun timeStampToMSS(context: Context, position: Long): String {
                val totalSeconds = Math.floor(position / 1E3).toInt()
                val minutes = totalSeconds/60
                val remainingSeconds = totalSeconds - (minutes * 60)
                return if (position < 0) context.getString(R.string.duration_unkown)
     ...
     * (i.e. play/pause button or black)
     */
     ...
     * means the currently active item has changed. As a result, the new and potentially old item
     * (if there was an old item), both need to ahve their [MediaItemData.playbackRes] changed
     * i.e. play/pause button or blank
     */

    private val mediaMetdataObserver = Observer<MediaMetadataCompat> {
     ...
     *
     * Specifically there's three things that are watched taht will cause the singled piece of
     ...
     * player, which can changed the [MediaItemData.playbackRes]s in the list.
     *
    ...
    private val musicServiceConnection = musicServiceConnection.also {
        ...
        it.nowPlaying.observeForever(mediaMetdataObserver)
    ...
    override fun onCleared() {
        ...
        musicServiceConnection.nowPlaying.removeObserver(mediaMetdataObserver)
    ...
    private fun updateState(
        ...
        if(mediaMetadata.duration != 0L && mediaMetadata.id != null){
            val nowPlayMetadata = NowPlayingMetadata(
                mediaMetadata.id!!,
            ...
            this.mediaMetadata.postValue(nowPlayMetadata)
        }
