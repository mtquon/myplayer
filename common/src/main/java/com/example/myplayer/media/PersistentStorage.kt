...
import com.example.myplayer.media.extensions.asAlbumArtContentUri
...
class PersistentStorage private constructor(val context: Context){
    ...
    suspend fun saveRecentSong(description: MediaDescriptionCompat, position: Long){
        withContext(Dispatchers.IO){
             ...
             * recently played song. Artwork for these media controls should not be loaded from
             * the network as it may be too slow or unavailable immediately after boot. Instead we
             * convert the iconUri to point toi the Glide on-disk cache.
             *
             */
...
private const val PREFERENCES_NAME = "myplayer"
